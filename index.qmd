---
tbl-cap-location: top
---

# Introducción {.unnumbered}

El Saber Pro es un instrumento de evaluación estandarizada para la medición externa de la calidad de la educación superior que evalúa las competencias de los estudiantes que están próximos a culminar los distintos programas profesionales universitarios. Está diseñada para medir tanto habilidades genéricas como conociemientos específicos relacionados con la formación académica de cada estudiante. Los módulos genéricos son Lectura Crítica, Razonamiento Cuantitativo, Competencias Ciudadanas, Comunicación Escrita e Inglés.[^index-1]

[^index-1]: Para más información consulte <https://www.icfes.gov.co/acerca-del-examen-saber-pro>.

La prueba es de carácter obligatorio, va dirigida a estudiantes con un avance del 75% en su plan de estudios y su propósito es evaluar la calidad de la educación superior del país.

### Marco conceptual

Anteriormente ya se han hecho estudios utilizando los datos públicos del Saber Pro con el fin de caracterizar distintos grupos con respecto a sus puntajes. @ZambranoPortilla2022 realizaron un análisis descriptivo para caracterizar a los estudiantes de una universidad de frontera, diferenciando entre sus puntajes y facultades. Los resultados evidenciaban que los programas de la Facultad de Ciencias Básicas destacaban teniendo los promedios más altos en cada componente, junto a las carreras de la Facultad de la Salud, que presentaban promedios superiores a la media de la Universidad.

Por otro lado, @RodriguezGomezAriza2014, analizaron los resultados de la prueba Saber Pro aplicada en el año 2010, allí observaron una diferencia en el desempeño académico entre estudiantes en modalidades presenciales y a distancia, con mejores resultados para los primeros, así como también que la institución tiene un impacto significativo en el rendimiento académico universitario, aunque este efecto disminuye en la educación a distancia; adicionalmente evidenciaron una disparidad de género a favor de los hombres, y una asociación positiva entre el nivel socioeconómico y el rendimiento académico. Esto último también fue evidenciado por @JehangirGlassBerg2015 en su investigación con los resultados de las pruebas PISA (Programa para la Evaluación Internacional de los Estudiantes).

Para completar, @OviedoJimenez2019, en su estudio sobre los resultados obtenidos en las pruebas Saber Pro del 2016 de estudiantes de ingeniería en Antioquia, destacaron una distinción evidente entre los estudiantes que lograron un desempeño sobresaliente en inglés, quienes generalmente pertenecen a un nivel socioeconómico más alto. Por otro lado, aquellos estudiantes que obtuvieron buenos resultados en lectura crítica suelen provenir de estratos socioeconómicos bajos y enfrentan costos de matrícula más económicos. Además, en el análisis de selección de factores se identificaron variables relevantes, como el número de dependientes, el enfoque pedagógico, la estabilidad del hogar, la naturaleza académica de la institución y aspectos económicos como la disponibilidad de electrodomésticos como microondas a gas y posesión de motocicleta.

Además de esto, @CuéllarGuerrero2016 propusieron un índice del nivel socioeconómico basado en los resultados de las pruebas Saber Pro, con el fin de que sea usado en estudios dónde la construcción de un índice sea necesario, y particularmente, en estudios acerca del desempeño académico en la educación superior. El índice fue construido en base a tres dimensiones, y para cada dimensión se utilizaron distintas variables, entre las cuales está el ingreso mensual de la familia, el puntaje SISBEN, la educación y ocupación de los padres, entre otras. Se concluyó que entre mayor era el nivel socioeconómico, los puntajes obtenidos eran más altos.

En la literatura revisada se han hecho diversos estudios tanto para caracterizar por facultades como por el nivel socioeconómico, pero no hay mucha información respecto a una caracterización enfocada de manera espacial.

### Justificación

Este estudio es realizado dada la necesidad de comprender y abordar las disparidades en el desempeño académico entre diferentes grupos de individuos, identificando los factores socioeconómicos, personales y académicos que influyen en los resultados de las pruebas Saber Pro. Estos exámenes son críticos para evaluar la calidad de la educación superior en Colombia y, por lo tanto, entender las variables que afectan los puntajes puede ayudar a informar políticas educativas más efectivas y equitativas. Además, al explorar la posible relación espacial entre los puntajes de las pruebas y la ubicación geográfica de los individuos, se podría obtener información relevante sobre las disparidades regionales en el acceso a la educación y sus efectos en los resultados académicos.

# Objetivos

## Objetivo general

Identificar patrones y relaciones entre variables socioeconómicas, personales y académicas que influyen en los puntajes obtenidos en las pruebas Saber Pro, con el fin de identificar grupos de individuos con características similares.

## Objetivos Específicos

1.  Analizar y comparar los puntajes obtenidos en cada área de conocimiento y el puntaje general, diferenciando entre el carácter académico de cada institución, grupos de referencia, costo de matrícula, estrato, edad y género.

2.  Explorar y analizar la posible relación entre las variables socioeconómicas, personales y académicas con los puntajes obtenidos en las pruebas Saber Pro.

3.  Buscar patrones que sugieran una relación espacial entre los puntajes de las pruebas y el lugar donde reside el individuo, comparándolo con las regiones geográficas de Colombia (Caribe, Pacífico, Orinoquía, Amazonía, Andina e Insular).

# Descripción de la tabla de datos

El conjunto de datos que se trabajarán para cumplir con los objetivos de este estudio son los resultados del examen Saber Pro del 2022-2, proporcionados por el Instituto Colombiano para la Evaluación de la Educación (ICFES). 

Los datos fueron tomados del cuestionario sociodemográfico que hace parte del formulario de inscripción al exámen Saber Pro, este cuestionario busca indagar por tres aspectos: información personal, académica y de citación, y socioeconómica.

La base de datos original cuenta con 127342 observaciones de 109 variables, de las cuales 29 son cuantitativas y hacen referencia a puntajes totales y parciales del examen, las otras 80 variables muestran información personal, académica y socioeconómica entre otras referentes a quienes presentaron la prueba.

```{r}
#| message: false
#| warning: false
#| echo: false

# Librerías
library(FactoClass)
library(plotly)
library(knitr)
library(readr)
library(DT)
library(dplyr)

# Carga de datos originales
saberpro <- read_delim("DATA_ICFES_SABERPROPROFESIONAL_2022-2.TXT", delim = "¬", escape_double = FALSE, trim_ws = TRUE)
#kable(head(saberpro), digits=2) 
datatable(head(saberpro, 60), style = "bootstrap4",
          options = list(pageLength = 6))
```

Puede encontrar la documentación de estos datos [aquí](https://github.com/SofiaBello27/Consumers/blob/main/1.%20Documentaci%C3%B3n_Saber_Pro.pdf) y el diccionario de datos [aquí](https://github.com/SofiaBello27/Consumers/blob/main/Gen%C3%A9ricas_SaberPro_2012_2020.pdf)

# Recodificación de variables

Dados los objetivos de este proyecto, se redujo la base de datos a un conjunto con 127342 observaciones de 34 variables que ayudan a cumplir dichos objetivos. Para la mayoría de variables cualitativas, algunos individuos tenían como valor "NA", por lo que se creo una categoría extra en cada variable llamada "Desconocido".

Para la variable que hace referencia al departamento de residencia (ESTU_DEPTO_RESIDE), todos los valores que hacían referencia a lugares fuera de Colombia, como lo son "Nueva York", "Sao Paulo", "Trenton", entre otras, fueron recodificados como "Extranjero", la cual es una categoría que ya existía en la variable.

Para la variable que hace referencia al semestre que cursa el estudiante (ESTU_SEMESTRECURSA), se creo una categoría en la que se agrupan los valores menores o iguales a 6 semestres llamada "06 o menos".

La mayoría de las variables cualitativas, los nombres de sus categorías fueron reducidos para mejorar la visualización de ciertos gráficos. Para más detalles puede ver el diccionario de datos para este proyecto en @sec-diccionario.

Si desea conocer más información de las variables originales y los valores que pueden tomar, puede consultar el diccionario de variables originales disponible [aquí](https://github.com/SofiaBello27/Consumers/blob/main/Gen%C3%A9ricas_SaberPro_2012_2020.pdf).

```{r}
#| echo: false
# Datos con las variables necesarias para el estudio
data <- saberpro[,-c(1,2,5:9,13:15,17:21,23:26,28:34,49:54,56:62,65,67:70,72:75,77:82,84:86,88:90,92:94,96:98,100:102,104:109)]
```

```{r}
#| echo: false
#| results: hide
# genero
sum(is.na(data$ESTU_GENERO)) # hay 5 NA'S
data$ESTU_GENERO <- ifelse(is.na(data$ESTU_GENERO), "Desconocido", data$ESTU_GENERO)
data$ESTU_GENERO <- as.factor(data$ESTU_GENERO)
table(data$ESTU_GENERO)

# edad
# esta la quitamos
library(stringr)
edad <- str_sub(data$ESTU_FECHANACIMIENTO,7,10)
edad<-as.numeric(edad)
edades<-2023-edad

# etnia
sum(is.na(data$ESTU_TIENEETNIA)) # 775 na's
data$ESTU_TIENEETNIA <- ifelse(is.na(data$ESTU_TIENEETNIA), "Desconocido", data$ESTU_TIENEETNIA)
data$ESTU_TIENEETNIA <- as.factor(data$ESTU_TIENEETNIA)
table(data$ESTU_TIENEETNIA)

# depto
data$ESTU_DEPTO_RESIDE<-ifelse(data$ESTU_DEPTO_RESIDE %in% c("BARCELONA","BUENOS AIRES","FRANKFURT","HOUSTON","LIMA","MIAMI","NUEVA YORK","PARIS","QUEBEC","ROMA","SACRAMENTO","SANTIAGO DE CHILE","SAO PAULO","SEUL","TRENTON"),"EXTRANJERO",data$ESTU_DEPTO_RESIDE)
sum(is.na(data$ESTU_DEPTO_RESIDE))
length(unique(data$ESTU_DEPTO_RESIDE)) # hay 35 pq son mas los extranjeros y NAs
data$ESTU_DEPTO_RESIDE <- ifelse(is.na(data$ESTU_DEPTO_RESIDE), "Desconocido", data$ESTU_DEPTO_RESIDE)
data$ESTU_DEPTO_RESIDE <- as.factor(data$ESTU_DEPTO_RESIDE)
data <- data %>%
  mutate(ESTU_DEPTO_RESIDE = case_when(
    ESTU_DEPTO_RESIDE == "BOGOTÁ" ~ ESTU_DEPTO_RESIDE,
    ESTU_DEPTO_RESIDE == "Desconocido" ~ "Desconocido",
    TRUE ~ "OTRO"
  ))
data$ESTU_DEPTO_RESIDE <- as.factor(data$ESTU_DEPTO_RESIDE)
table(data$ESTU_DEPTO_RESIDE)


# estado civil
sum(is.na(data$ESTU_ESTADOCIVIL)) # 580 na's
data$ESTU_ESTADOCIVIL <- ifelse(is.na(data$ESTU_ESTADOCIVIL), "Desconocido", ifelse(data$ESTU_ESTADOCIVIL=="Casado","C",ifelse(data$ESTU_ESTADOCIVIL=="Separado y/o Viudo","SyoV",ifelse(data$ESTU_ESTADOCIVIL=="Soltero","S","UL"))))
data$ESTU_ESTADOCIVIL <- as.factor(data$ESTU_ESTADOCIVIL)
table(data$ESTU_ESTADOCIVIL)

# valor matricula
## recodificamos para que quede como factor
sum(is.na(data$ESTU_VALORMATRICULAUNIVERSIDAD)) # 2194 na's
data$ESTU_VALORMATRICULAUNIVERSIDAD <- ifelse(is.na(data$ESTU_VALORMATRICULAUNIVERSIDAD), "Desconocido", ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Menos de 500 mil","m500",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Entre 500 mil y menos de 1 millón","e500m1",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Entre 1 millón y menos de 2.5 millones","e1m2.5",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Entre 2.5 millones y menos de 4 millones","e2.5m4",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Entre 4 millones y menos de 5.5 millones","e4m5.5",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Entre 5.5 millones y menos de 7 millones","e5.5m7",ifelse(data$ESTU_VALORMATRICULAUNIVERSIDAD=="Más de 7 millones","M7","NP"))))))))
data$ESTU_VALORMATRICULAUNIVERSIDAD <- factor(data$ESTU_VALORMATRICULAUNIVERSIDAD, levels = c("m500","e500m1", "e1m2.5", "e2.5m4","e4m5.5","e5.5m7","M7","NP","Desconocido"))
table(data$ESTU_VALORMATRICULAUNIVERSIDAD)

# capacitacion del examen
sum(is.na(data$ESTU_COMOCAPACITOEXAMENSB11)) # 2069 na's
data$ESTU_COMOCAPACITOEXAMENSB11 <- ifelse(is.na(data$ESTU_COMOCAPACITOEXAMENSB11), "Desconocido", ifelse(data$ESTU_COMOCAPACITOEXAMENSB11=="No realizó ninguna prueba de preparación","Ninguna",ifelse(data$ESTU_COMOCAPACITOEXAMENSB11=="Repasó por cuenta propia","CuentaPropia","Curso")))
data$ESTU_COMOCAPACITOEXAMENSB11 <- as.factor(data$ESTU_COMOCAPACITOEXAMENSB11)
table(data$ESTU_COMOCAPACITOEXAMENSB11)


# semestre que cursa
sum(is.na(data$ESTU_SEMESTRECURSA)) # 2194 na's
data$ESTU_SEMESTRECURSA<-ifelse(data$ESTU_SEMESTRECURSA %in% c("01","02","03","04","05","06"),"06m",ifelse(data$ESTU_SEMESTRECURSA=="12 o más","12M",data$ESTU_SEMESTRECURSA))
data$ESTU_SEMESTRECURSA <- ifelse(is.na(data$ESTU_SEMESTRECURSA), "Desconocido", data$ESTU_SEMESTRECURSA)
data$ESTU_SEMESTRECURSA <- as.factor(data$ESTU_SEMESTRECURSA)
table(data$ESTU_SEMESTRECURSA)

# educacion papa
sum(is.na(data$FAMI_EDUCACIONPADRE)) # 4738 na's
data$FAMI_EDUCACIONPADRE <- ifelse(is.na(data$FAMI_EDUCACIONPADRE), "Desconocido", ifelse(data$FAMI_EDUCACIONPADRE=="Educación profesional completa","EPC",ifelse(data$FAMI_EDUCACIONPADRE=="Educación profesional incompleta","EPI",ifelse(data$FAMI_EDUCACIONPADRE=="No Aplica","NAp",ifelse(data$FAMI_EDUCACIONPADRE=="No sabe","NS",ifelse(data$FAMI_EDUCACIONPADRE=="Postgrado","Post",ifelse(data$FAMI_EDUCACIONPADRE=="Primaria completa","PC",ifelse(data$FAMI_EDUCACIONPADRE=="Primaria incompleta","PI",ifelse(data$FAMI_EDUCACIONPADRE=="Secundaria (Bachillerato) completa","BC",ifelse(data$FAMI_EDUCACIONPADRE=="Secundaria (Bachillerato) incompleta","BI",ifelse(data$FAMI_EDUCACIONPADRE=="Técnica o tecnológica completa","TTC",ifelse(data$FAMI_EDUCACIONPADRE=="Técnica o tecnológica incompleta","TTI",data$FAMI_EDUCACIONPADRE))))))))))))
data$FAMI_EDUCACIONPADRE <- as.factor(data$FAMI_EDUCACIONPADRE)
table(data$FAMI_EDUCACIONPADRE)

# educacion mama
sum(is.na(data$FAMI_EDUCACIONMADRE)) # 4886 na's
data$FAMI_EDUCACIONMADRE <- ifelse(is.na(data$FAMI_EDUCACIONMADRE), "Desconocido", ifelse(data$FAMI_EDUCACIONMADRE=="Educación profesional completa","EPC",ifelse(data$FAMI_EDUCACIONMADRE=="Educación profesional incompleta","EPI",ifelse(data$FAMI_EDUCACIONMADRE=="No Aplica","NAp",ifelse(data$FAMI_EDUCACIONMADRE=="No sabe","NS",ifelse(data$FAMI_EDUCACIONMADRE=="Postgrado","Post",ifelse(data$FAMI_EDUCACIONMADRE=="Primaria completa","PC",ifelse(data$FAMI_EDUCACIONMADRE=="Primaria incompleta","PI",ifelse(data$FAMI_EDUCACIONMADRE=="Secundaria (Bachillerato) completa","BC",ifelse(data$FAMI_EDUCACIONMADRE=="Secundaria (Bachillerato) incompleta","BI",ifelse(data$FAMI_EDUCACIONMADRE=="Técnica o tecnológica completa","TTC",ifelse(data$FAMI_EDUCACIONMADRE=="Técnica o tecnológica incompleta","TTI",data$FAMI_EDUCACIONMADRE))))))))))))
data$FAMI_EDUCACIONMADRE <- as.factor(data$FAMI_EDUCACIONMADRE)
table(data$FAMI_EDUCACIONMADRE)

# ocupacion papa
sum(is.na(data$FAMI_OCUPACIONPADRE)) # 2060 na's
data$FAMI_OCUPACIONPADRE <- ifelse(is.na(data$FAMI_OCUPACIONPADRE), "Desconocido", ifelse(data$FAMI_OCUPACIONPADRE=="Empleado con cargo como director o gerente general","EDGG",ifelse(data$FAMI_OCUPACIONPADRE=="Empleado de nivel auxiliar o administrativo","ENAA",ifelse(data$FAMI_OCUPACIONPADRE=="Empleado de nivel directivo","END",ifelse(data$FAMI_OCUPACIONPADRE=="Empleado de nivel técnico o profesional","ENTP",ifelse(data$FAMI_OCUPACIONPADRE=="Empleado obrero u operario","EOO",ifelse(data$FAMI_OCUPACIONPADRE=="Empresario","Emp",ifelse(data$FAMI_OCUPACIONPADRE=="Hogar","H",ifelse(data$FAMI_OCUPACIONPADRE=="Otra actividad u ocupación","OAO", ifelse(data$FAMI_OCUPACIONPADRE=="Pensionado","Pen",ifelse(data$FAMI_OCUPACIONPADRE=="Pequeño empresario","PE",ifelse(data$FAMI_OCUPACIONPADRE=="Profesional independiente","PI","TCP"))))))))))))
data$FAMI_OCUPACIONPADRE <- as.factor(data$FAMI_OCUPACIONPADRE)
table(data$FAMI_OCUPACIONPADRE)

# ocupacion mama
sum(is.na(data$FAMI_OCUPACIONMADRE)) # 2060 na's
data$FAMI_OCUPACIONMADRE <- ifelse(is.na(data$FAMI_OCUPACIONMADRE), "Desconocido", ifelse(data$FAMI_OCUPACIONMADRE=="Empleado con cargo como director o gerente general","EDGG",ifelse(data$FAMI_OCUPACIONMADRE=="Empleado de nivel auxiliar o administrativo","ENAA",ifelse(data$FAMI_OCUPACIONMADRE=="Empleado de nivel directivo","END",ifelse(data$FAMI_OCUPACIONMADRE=="Empleado de nivel técnico o profesional","ENTP",ifelse(data$FAMI_OCUPACIONMADRE=="Empleado obrero u operario","EOO",ifelse(data$FAMI_OCUPACIONMADRE=="Empresario","Emp",ifelse(data$FAMI_OCUPACIONMADRE=="Hogar","H",ifelse(data$FAMI_OCUPACIONMADRE=="Otra actividad u ocupación","OAO", ifelse(data$FAMI_OCUPACIONMADRE=="Pensionado","Pen",ifelse(data$FAMI_OCUPACIONMADRE=="Pequeño empresario","PE",ifelse(data$FAMI_OCUPACIONMADRE=="Profesional Independiente","PI","TCP"))))))))))))
data$FAMI_OCUPACIONMADRE <- as.factor(data$FAMI_OCUPACIONMADRE)
table(data$FAMI_OCUPACIONMADRE)

# estrato
sum(is.na(data$FAMI_ESTRATOVIVIENDA)) # 5531 na's
data$FAMI_ESTRATOVIVIENDA <- ifelse(is.na(data$FAMI_ESTRATOVIVIENDA), "Desconocido", data$FAMI_ESTRATOVIVIENDA)
data$FAMI_ESTRATOVIVIENDA <- as.factor(data$FAMI_ESTRATOVIVIENDA)
table(data$FAMI_ESTRATOVIVIENDA)


# internet
sum(is.na(data$FAMI_TIENEINTERNET)) # 5697 na's
data$FAMI_TIENEINTERNET <- ifelse(is.na(data$FAMI_TIENEINTERNET), "Desconocido", data$FAMI_TIENEINTERNET)
data$FAMI_TIENEINTERNET <- as.factor(data$FAMI_TIENEINTERNET)
table(data$FAMI_TIENEINTERNET)

# computador
sum(is.na(data$FAMI_TIENECOMPUTADOR)) # 6163 na's
data$FAMI_TIENECOMPUTADOR <- ifelse(is.na(data$FAMI_TIENECOMPUTADOR), "Desconocido", data$FAMI_TIENECOMPUTADOR)
data$FAMI_TIENECOMPUTADOR <- as.factor(data$FAMI_TIENECOMPUTADOR)
table(data$FAMI_TIENECOMPUTADOR)

# lavadora
sum(is.na(data$FAMI_TIENELAVADORA)) # 6354 na's
data$FAMI_TIENELAVADORA <- ifelse(is.na(data$FAMI_TIENELAVADORA), "Desconocido", data$FAMI_TIENELAVADORA)
data$FAMI_TIENELAVADORA <- as.factor(data$FAMI_TIENELAVADORA)
table(data$FAMI_TIENELAVADORA)

# microogas
sum(is.na(data$FAMI_TIENEHORNOMICROOGAS)) # 7019 na's
data$FAMI_TIENEHORNOMICROOGAS <- ifelse(is.na(data$FAMI_TIENEHORNOMICROOGAS), "Desconocido", data$FAMI_TIENEHORNOMICROOGAS)
data$FAMI_TIENEHORNOMICROOGAS <- as.factor(data$FAMI_TIENEHORNOMICROOGAS)
table(data$FAMI_TIENEHORNOMICROOGAS)

# tv
sum(is.na(data$FAMI_TIENESERVICIOTV)) # 5801 na's
data$FAMI_TIENESERVICIOTV <- ifelse(is.na(data$FAMI_TIENESERVICIOTV), "Desconocido", data$FAMI_TIENESERVICIOTV)
data$FAMI_TIENESERVICIOTV <- as.factor(data$FAMI_TIENESERVICIOTV)
table(data$FAMI_TIENESERVICIOTV)

# auto
sum(is.na(data$FAMI_TIENEAUTOMOVIL)) # 7187 na's
data$FAMI_TIENEAUTOMOVIL <- ifelse(is.na(data$FAMI_TIENEAUTOMOVIL), "Desconocido", data$FAMI_TIENEAUTOMOVIL)
data$FAMI_TIENEAUTOMOVIL <- as.factor(data$FAMI_TIENEAUTOMOVIL)
table(data$FAMI_TIENEAUTOMOVIL)

# moto
sum(is.na(data$FAMI_TIENEMOTOCICLETA)) # 7525 na's
data$FAMI_TIENEMOTOCICLETA <- ifelse(is.na(data$FAMI_TIENEMOTOCICLETA), "Desconocido", data$FAMI_TIENEMOTOCICLETA)
data$FAMI_TIENEMOTOCICLETA <- as.factor(data$FAMI_TIENEMOTOCICLETA)
table(data$FAMI_TIENEMOTOCICLETA)

# consola
sum(is.na(data$FAMI_TIENECONSOLAVIDEOJUEGOS)) # 7993 na's
data$FAMI_TIENECONSOLAVIDEOJUEGOS <- ifelse(is.na(data$FAMI_TIENECONSOLAVIDEOJUEGOS), "Desconocido", data$FAMI_TIENECONSOLAVIDEOJUEGOS)
data$FAMI_TIENECONSOLAVIDEOJUEGOS <- as.factor(data$FAMI_TIENECONSOLAVIDEOJUEGOS)
table(data$FAMI_TIENECONSOLAVIDEOJUEGOS)

# horas trabaja
sum(is.na(data$ESTU_HORASSEMANATRABAJA)) # 6867 na's
data$ESTU_HORASSEMANATRABAJA <- ifelse(is.na(data$ESTU_HORASSEMANATRABAJA), "Desconocido", data$ESTU_HORASSEMANATRABAJA)
data$ESTU_HORASSEMANATRABAJA <- factor(data$ESTU_HORASSEMANATRABAJA, levels = c("0", "Menos de 10 horas", "Entre 11 y 20 horas", "Entre 21 y 30 horas", "Más de 30 horas", "Desconocido"))
table(data$ESTU_HORASSEMANATRABAJA)

# grupo de referencia
sum(is.na(data$GRUPOREFERENCIA)) # 0 na's
data$GRUPOREFERENCIA<-ifelse(data$GRUPOREFERENCIA=="ADMINISTRACIÓN Y AFINES","AdmYAfin",ifelse(data$GRUPOREFERENCIA=="ARQUITECTURA Y URBANISMO","ArqYUrb",ifelse(data$GRUPOREFERENCIA=="BELLAS ARTES Y DISEÑO","BelArtYDis",ifelse(data$GRUPOREFERENCIA=="CIENCIAS AGROPECUARIAS","CienAgr",ifelse(data$GRUPOREFERENCIA=="CIENCIAS MILITARES Y NAVALES","CienMilYNav",ifelse(data$GRUPOREFERENCIA=="CIENCIAS NATURALES Y EXACTAS","CienNatYExac",ifelse(data$GRUPOREFERENCIA=="CIENCIAS SOCIALES","CienSoc",ifelse(data$GRUPOREFERENCIA=="COMUNICACIÓN, PERIODISMO Y PUBLICIDAD","ComuPerYPub",ifelse(data$GRUPOREFERENCIA=="CONTADURÍA Y AFINES","ContYAfin",ifelse(data$GRUPOREFERENCIA=="DERECHO","Derecho",ifelse(data$GRUPOREFERENCIA=="ECONOMÍA","Economía",ifelse(data$GRUPOREFERENCIA=="ECONOMÍA, ADMINISTRACIÓN, CONTADURÍA Y AFINES - UNIVERSITARIA","EcAdContYAfinU",ifelse(data$GRUPOREFERENCIA=="EDUCACIÓN","Educación",ifelse(data$GRUPOREFERENCIA=="ENFERMERÍA","Enfermeria",ifelse(data$GRUPOREFERENCIA=="GRUPO REFERENCIA NACIONAL","GrupRefNal", ifelse(data$GRUPOREFERENCIA=="GRUPO REFERENCIA NACIONAL UNIVERSITARIO","GrupRefNalU", ifelse(data$GRUPOREFERENCIA=="HUMANIDADES","Hum", ifelse(data$GRUPOREFERENCIA=="INGENIERÍA","Inge", ifelse(data$GRUPOREFERENCIA=="MEDICINA","Med", ifelse(data$GRUPOREFERENCIA=="PSICOLOGÍA","Psic", ifelse(data$GRUPOREFERENCIA=="RECREACIÓN Y DEPORTES","RecYDep", "salud")))))))))))))))))))))
data$GRUPOREFERENCIA<- as.factor(data$GRUPOREFERENCIA)
table(data$GRUPOREFERENCIA)
str(data$GRUPOREFERENCIA) # 22 niveles

# metodo
sum(is.na(data$ESTU_METODO_PRGM)) # 0 na's
data$ESTU_METODO_PRGM <- ifelse(data$ESTU_METODO_PRGM == "DISTANCIA VITUAL", "distVirtual", ifelse(data$ESTU_METODO_PRGM=="DISTANCIA","distancia","presencial"))
data$ESTU_METODO_PRGM<- as.factor(data$ESTU_METODO_PRGM)
table(data$ESTU_METODO_PRGM)

# caracter academico institucion
sum(is.na(data$INST_CARACTER_ACADEMICO)) # 0 na's
data$INST_CARACTER_ACADEMICO<-ifelse(data$INST_CARACTER_ACADEMICO=="INSTITUCIÓN TECNOLÓGICA","InstTec",ifelse(data$INST_CARACTER_ACADEMICO=="INSTITUCIÓN UNIVERSITARIA","InstUni", ifelse(data$INST_CARACTER_ACADEMICO=="TÉCNICA PROFESIONAL","TecProf", "Universidad")))
data$INST_CARACTER_ACADEMICO<- as.factor(data$INST_CARACTER_ACADEMICO)
table(data$INST_CARACTER_ACADEMICO)

names(data)[29:33]<-c("cuanti","lec","ciu","ing","esc")
```

## Diccionario de datos para el análisis {#sec-diccionario}

### Variables cualitativas

+----+--------------------------------+--------+----------------------------------------------------------+
| Col| Variable                       | Tipo   | Categorías                                               |
+====+================================+========+==========================================================+
| 1  | ESTU_GENERO                    | Nominal|- F: Femenino                                             |
|    |                                |        |- M: Masculino                                            |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 2  | ESTU_FECHANACIMIENTO           | Nominal|                                                          |
+----+--------------------------------+--------+----------------------------------------------------------+
| 3  | ESTU_TIENEETNIA                | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 4  | ESTU_DEPTO_RESIDE              | Nominal|- BOGOTÁ                                                  |
|    |                                |        |- OTRO                                                    |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 5  | ESTU_COD_RESIDE_DEPTO          | Nominal|                                                          |
+----+--------------------------------+--------+----------------------------------------------------------+
| 6  | ESTU_ESTADOCIVIL               | Nominal|- S: Soltero                                              |
|    |                                |        |- UL: Unión libre                                         |
|    |                                |        |- C: Casado                                               |
|    |                                |        |- SyoV: Separado y/o viudo                                |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 7  | ESTU_VALORMATRICULAUNIVERSIDAD | Nominal|- m500: Menos de 500 mil                                  |
|    |                                |        |- e500m1: Entre 500 mil y menos de 1 millón               |
|    |                                |        |- e1m2.5: Entre 1 millón y menos de 2.5 millones          |
|    |                                |        |- e2.5m4: Entre 2.5 millones y menos de 4 millones        |
|    |                                |        |- e4m5.5: Entre 4 millones y menos de 5.5 millones        |
|    |                                |        |- e5.5m7: Entre 5.5 millones y menos de 7 millones        |
|    |                                |        |- M7: Más de 7 millones                                   |
|    |                                |        |- NP: No paga                                             |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 8  | ESTU_COMOCAPACITOEXAMENSB11    | Nominal|- Ninguna: No realizó ninguna prueba de preparación       |
|    |                                |        |- CuentaPropia: Repasó por cuenta propia                  |
|    |                                |        |- Curso: Tomó un curso de preparación                     |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 9  | ESTU_SEMESTRECURSA             | Nominal|- 06m: 6 semestres o menos                                |
|    |                                |        |- 07                                                      |
|    |                                |        |- 08                                                      |
|    |                                |        |- 09                                                      |
|    |                                |        |- 10                                                      |
|    |                                |        |- 11                                                      |
|    |                                |        |- 12M: 12 semestre o más                                  |
|    |                                |        |- Desconocido                                             |   
+----+--------------------------------+--------+----------------------------------------------------------+
| 10 | FAMI_EDUCACIONPADRE            | Nominal|- EPC: Educación profesional completa                     |
|    |                                |        |- EPI: Educación profesional incompleta                   |
|    |                                |        |- Ninguno                                                 |
|    |                                |        |- NAp: No aplica                                          |
|    |                                |        |- NS: No sabe                                             |
|    |                                |        |- Post: Postgrado                                         |
|    |                                |        |- PC: Primaria completa                                   |
|    |                                |        |- PI: Primaria incompleta                                 |
|    |                                |        |- BC: Bachillerato completo                               |
|    |                                |        |- BI: Bachillerato incompleto                             |
|    |                                |        |- TTC: Técnica o tecnológica completa                     |
|    |                                |        |- TTI: Técnica o tecnológica incompleta                   |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 11 | FAMI_EDUCACIONMADRE            | Nominal|- EPC: Educación profesional completa                     |
|    |                                |        |- EPI: Educación profesional incompleta                   |
|    |                                |        |- Ninguno                                                 |
|    |                                |        |- NAp: No aplica                                          |
|    |                                |        |- NS: No sabe                                             |
|    |                                |        |- Post: Postgrado                                         |
|    |                                |        |- PC: Primaria completa                                   |
|    |                                |        |- PI: Primaria incompleta                                 |
|    |                                |        |- BC: Bachillerato completo                               |
|    |                                |        |- BI: Bachillerato incompleto                             |
|    |                                |        |- TTC: Técnica o tecnológica completa                     |
|    |                                |        |- TTI: Técnica o tecnológica incompleta                   |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 12 | FAMI_OCUPACIONPADRE            | Nominal|- EDGG:Empleado con cargo como director o gerente general |
|    |                                |        |- ENAA: Empleado de nivel auxiliar o administrativo       |
|    |                                |        |- END: Empleado de nivel directivo                        |
|    |                                |        |- ENTP: Empleado de nivel técnico o profesional           |
|    |                                |        |- EOO: Empleado obrero u operario                         |
|    |                                |        |- Emp: Empresario                                         |
|    |                                |        |- H: Hogar                                                |
|    |                                |        |- OAO: Otra actividad u ocupación                         |
|    |                                |        |- Pen: Pensionado                                         |
|    |                                |        |- PE: Pequeño empresario                                  |
|    |                                |        |- PI: Profesional independiente                           |
|    |                                |        |- TCP: Trabajador por cuenta propia                       |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 13 | FAMI_OCUPACIONMADRE            | Nominal|- EDGG:Empleado con cargo como director o gerente general |
|    |                                |        |- ENAA: Empleado de nivel auxiliar o administrativo       |
|    |                                |        |- END: Empleado de nivel directivo                        |
|    |                                |        |- ENTP: Empleado de nivel técnico o profesional           |
|    |                                |        |- EOO: Empleado obrero u operario                         |
|    |                                |        |- Emp: Empresario                                         |
|    |                                |        |- H: Hogar                                                |
|    |                                |        |- OAO: Otra actividad u ocupación                         |
|    |                                |        |- Pen: Pensionado                                         |
|    |                                |        |- PE: Pequeño empresario                                  |
|    |                                |        |- PI: Profesional independiente                           |
|    |                                |        |- TCP: Trabajador por cuenta propia                       |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 14 | FAMI_ESTRATOVIVIENDA           | Nominal|- Sin Estrato                                             |
|    |                                |        |- Estrato 1                                               |
|    |                                |        |- Estrato 2                                               |
|    |                                |        |- Estrato 3                                               |
|    |                                |        |- Estrato 4                                               |
|    |                                |        |- Estrato 5                                               |
|    |                                |        |- Estrato 6                                               |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 15 | FAMI_TIENEINTERNET             | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 16 | FAMI_TIENECOMPUTADOR           | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 17 | FAMI_TIENELAVADORA             | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 18 | FAMI_TIENEHORNOMICROOGAS       | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 19 | FAMI_TIENESERVICIOTV           | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 20 | FAMI_TIENEAUTOMOVIL            | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 21 | FAMI_TIENEMOTOCICLETA          | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 22 | FAMI_TIENECONSOLAVIDEOJUEGOS   | Nominal|- Si                                                      |
|    |                                |        |- No                                                      |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 23 | ESTU_HORASSEMANATRABAJA        | Nominal|- Menos de 10 horas                                       |
|    |                                |        |- Entre 11 y 20 horas                                     |
|    |                                |        |- Entre 21 y 30 horas                                     |
|    |                                |        |- Más de 30 horas                                         |
|    |                                |        |- 0                                                       |
|    |                                |        |- Desconocido                                             |
+----+--------------------------------+--------+----------------------------------------------------------+
| 24 | INST_NOMBRE_INSTITUCION        | Nominal|                                                          |
+----+--------------------------------+--------+----------------------------------------------------------+
| 25 | ESTU_PRGM_ACADEMICO            | Nominal|                                                          |
+----+--------------------------------+--------+----------------------------------------------------------+
| 26 | GRUPOREFERENCIA                | Nominal|- AdmYAfin: Administración y afines                       |
|    |                                |        |- ArqYUrb: Arquitectura y urbanismo                       |
|    |                                |        |- BelArtYDis: Bellas artes y diseño                       |
|    |                                |        |- CienAgr: Ciencias agrarias                              |
|    |                                |        |- CienMilYNav: Ciencias militares y navales               |
|    |                                |        |- CienNatYExac: Ciencias naturales y exactas              |
|    |                                |        |- CienSoc: Ciencias sociales                              |
|    |                                |        |- ComuPerYPub: Comunicación, periodismo y publicidad      |
|    |                                |        |- ContYAfin: Contaduría y afines                          |
|    |                                |        |- Derecho                                                 |
|    |                                |        |- Economía                                                |
|    |                                |        |- EcAdContYAfinU: Econ.,admin.,contad. y afines Univer.   |
|    |                                |        |- Educación                                               |
|    |                                |        |- Enfermería                                              |
|    |                                |        |- GrupRefNal: Grupo referencia nacional                   |
|    |                                |        |- GrupRefNalU: Grupo referencia nacional universitaria    |
|    |                                |        |- Hum: Humanidades                                        |
|    |                                |        |- Inge: Ingeniería                                        |
|    |                                |        |- Med: Medicina                                           |
|    |                                |        |- Pisc: Psicología                                        |
|    |                                |        |- RecYDep: Recreación y deportes                          |
|    |                                |        |- salud                                                   |
+----+--------------------------------+--------+----------------------------------------------------------+
| 27 | ESTU_METODO_PRGM               | Nominal|- presencial: Presencial                                  |
|    |                                |        |- distancia: Distancia                                    |
|    |                                |        |- distVirtual: Distancia virtual                          |
+----+--------------------------------+--------+----------------------------------------------------------+
| 28 |INST_CARACTER_ACADEMICO         | Nominal|- InstTec: Institución tecnológica                        |
|    |                                |        |- InstUni: Institución universitaria                      |
|    |                                |        |- TecProf: Técnica profesional                            |
|    |                                |        |- Universidad                                             |
+----+--------------------------------+--------+----------------------------------------------------------+

: Diccionario variables cualitativas

### Varibles cuantitativas

+----+----------------+---------------------+-----------------------------+
| Col| Variable       | Tipo                | Nombre original             |
+====+================+=====================+=============================+
| 29 | cuanti         | Discreta, intervalo | MOD_RAZONA_CUANTITAT_PUNT   |
+----+----------------+---------------------+-----------------------------+
| 30 | lec            | Discreta, intervalo | MOD_LECTURA_CRITICA_PUNT    |
+----+----------------+---------------------+-----------------------------+
| 31 | ciu            | Discreta, intervalo | MOD_COMPETEN_CIUDADA_PUNT   |
+----+----------------+---------------------+-----------------------------+
| 32 | ing            | Discreta, intervalo | MOD_INGLES_PUNT             |
+----+----------------+---------------------+-----------------------------+
| 33 | esc            | Discreta, intervalo | MOD_COMUNI_ESCRITA_PUNT     |
+----+----------------+---------------------+-----------------------------+
| 34 | PUNT_GLOBAL    | Discreta, intervalo | PUNT_GLOBAL                 |
+----+----------------+---------------------+-----------------------------+

: Diccionario variables cuantitativas

### Visualización de los datos

Entonces, los datos utilizados para este análisis se ven así:

```{r}
#| echo: false
#kable(head(data), digits=2) 
datatable(head(data, 60), style = "bootstrap4",
          options = list(pageLength = 6))
```
