# Análisis de Correspondecias Múltiples

A continuación se muestra el Análisis de Correspondencias Múltiples (ACM) de los datos SaberPro 2022-2.

Se realiza un ACM para cada una de los distintos tipos de variables que cuenta la base de datos (Variables relacionadas con la información personal, Variables relacionadas con la información académica y Variables relacionadas con la información socioeconómica). En cada uno de los ACP se muestra la variable cuantitativa correspondiente al puntaje global como variable suplementaria.
...
```
Como en el ACM existen ejes que se consideran "parásitos", se utiliza el \textbf{Criterio de Benzecri} para determinar con cuantos ejes realizar el ACM. Luego de aplicar dicho criterio, se obtiene la siguiente inercia para los ejes:
...
La inercia en cada eje luego de aplicar el cirterio de Benzecri es
...
Se utiliza el \textbf{Criterio de Benzecri} para determinar con cuantos ejes realizar el ACM. Luego de aplicar dicho criterio, se obtiene la siguiente inercia para los ejes:
...
```{r}
#| echo: false
fviz_contrib(acm, choice = "var", axes = 3, top=13) # top=6 para que muestre quienes contribuyen más
```
